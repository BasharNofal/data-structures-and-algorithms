...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  return input.map(arr => arr.filter(element => (!(element % 5) && Number.isInteger(element)))).map(arr => arr.map(element => Math.pow(2, element)));
}
...
let findMaleAndFemale = (data) => {
  // // Solution code here...
  return data.filter(obj => (obj.gender === 'male' || obj.gender === 'female')).map(malesAndFemalesObjs => malesAndFemalesObjs.name).join(' and ');
}
...
let findShortest = (data) => {
  // Solution code here...
  let arrOfHeights = data.map(obj => obj.height).sort((a, b) => a - b);
  return data.filter(obj => obj.height === arrOfHeights[0])[0].name;
};
