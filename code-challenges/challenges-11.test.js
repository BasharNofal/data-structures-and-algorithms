...
const toLastNames = people => {
  ...
  let arrOfNames = [];
  people.map(names =>{
    arrOfNames.push(names.firstName + ' '+ names.lastName)
  })
  return arrOfNames;
};
...
const hasNumber = (string) => {
  // Solution code here...
  let regex = /^[a-z]+[0-9]+/i;
  if (string.toString().match(regex)){
    return true;
  }
  return false;
};
...
const validatePhoneNumber = (phoneNumber) => {
  // Solution code here...
  let regex = /^([0-9]{3}|\([0-9]{3}\))[^0-9]?([0-9]{3})[^0-9]?([0-9]{4})$/;
