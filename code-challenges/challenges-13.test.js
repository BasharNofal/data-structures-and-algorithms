...
const longestString = (arr) => {
  // Solution code here...
  return arr.indexOf((arr.reduce((accu, str) => {
    return (accu.length > str.length ? accu : str)
  }, 0)));
};
...
const firstLetters = (arr) => {
  ...
  arr.forEach(str => {
    arrOfFirstLetter.push(str.charAt(0));
  });
  return arrOfFirstLetter;
};
...
const findHappiness = (arr) => {
  ...
  arr.forEach(str => {
    if (str.includes(':)')) {
      arrOfHappiness.push(str);
    }
  })
  return arrOfHappiness;
};
...
const standardizePhoneNumbers = (arr) => {
  ...
  let arrOfNums = [];
  arr.forEach(num => {
    arrOfNums.push(num.replace(/\D/g, ''));
  });
  return arrOfNums;
};
